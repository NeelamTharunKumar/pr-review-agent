...
"""
        ...
        if getattr(context, "truncated_files", None):
            body += """
---
...
"""
            for tf in context.truncated_files:
                total = tf.get("total_lines") or 0
                omitted = tf.get("omitted_lines") or 0
                shown = tf.get("shown_lines") or 0
                pct = round((omitted / total) * 100) if total else 0
                body += (
                    f"- `{tf.get('filename', 'unknown')}`: "
                    f"Only first {shown} lines reviewed ({pct}% omitted)\n"
                )
...
"""
    ...
    def _build_inline_comments(
        ...
        for comment in valid_comments:
            if not comment.filename or comment.line is None or comment.line < 1:
                continue
            ...
            conf_pct = round(comment.confidence * 100)
