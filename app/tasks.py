...
def review_pr(self, repo_name: str, pr_number: int):
    ...
    logger.info(f"[Celery] Task review_pr started — Repo: {repo_name} | PR: #{pr_number}")
    ...
    try:
        ...
        logger.info(f"[Celery] Task review_pr completed — Repo: {repo_name} | PR: #{pr_number}")
        return {"status": "completed", "repo": repo_name, "pr": pr_number}

    except Exception as exc:
        logger.error(f"[Celery] Task review_pr failed — {type(exc).__name__}: {exc}")
...
def ingest_repo(self, repo_name: str):
    ...
    try:
        ...
        logger.error(f"[Celery] Task ingest_repo failed — {type(exc).__name__}: {exc}")
