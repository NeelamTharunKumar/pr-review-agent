...
class PRReview(Base):
    ...
    metrics = relationship(
        "EvaluationMetrics", back_populates="review", uselist=False, cascade="all, delete-orphan"
    )
...
class ReviewComment(Base):
    ...
    def __repr__(self):
        return f"<ReviewComment id={self.id} severity={self.severity} valid={self.is_valid_line}>"
...
class EvaluationMetrics(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    review_id = Column(Integer, ForeignKey("pr_reviews.id"), nullable=False, unique=True)
    ...
    review = relationship("PRReview", back_populates="metrics")
