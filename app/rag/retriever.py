...
class CodebaseRetriever:
    ...
    def retrieve_context(self, context: PRContext, n_results: int = 5) -> str:
        ...
        changed_paths = [f.filename for f in context.files if f.filename]
        ...
        if collection_count == 0:
            logger.warning(f"[Retriever] Collection for {context.repo_name} is empty")
            return ""

        vector_n = min(max(n_results * 3, 15), collection_count)
        vector_results = None
        if query_embedding:
            query_kwargs = {
            ...
            if changed_paths:
                query_kwargs["where"] = {"filepath": {"$nin": changed_paths}}
            try:
                ...
                try:
                    ...
                    logger.error(f"[Retriever] Vector query retry failed: {e2}")
        ...
        if bm25_data:
            ...
            bm25_results = [
                ...
                if corpus[i].get("filepath") not in changed_paths
            ]
