...
def test_poster_inline_comment_filtering():
    ...
    changed_file = ChangedFile(
        filename="src/auth.py", status="modified", additions=100, deletions=0, patch=diff
    )
    ...
    valid_comment_1 = ReviewComment(
        filename="src/auth.py", line=10, issue="Issue 1", suggestion="Fix 1", severity="critical"
    )
    valid_comment_2 = ReviewComment(
        filename="src/auth.py", line=50, issue="Issue 2", suggestion="Fix 2", severity="warning"
    )
    valid_comment_3 = ReviewComment(
        filename="src/auth.py", line=90, issue="Issue 3", suggestion="Fix 3", severity="suggestion"
    )
...
def test_poster_summary_count_filtering():
    ...
    warning = ReviewComment(
        filename="src/auth.py", line=20, issue="Warning issue", suggestion="Fix", severity="warning"
    )
    ...
    result = ReviewResult(
        overall_score=8, approved=True, summary="Good", comments=[critical, warning, suggestion]
    )

    print("\n--- Scenario: Filtering summary counts by severity ---")
    print("  Input: 1 critical, 1 warning, 1 suggestion (3 total)")
