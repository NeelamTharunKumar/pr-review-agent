...
def test_truncation_tracking():
    ...
    _prompt = build_user_prompt(context)
    ...
    assert truncated["total_lines"] == 1500, f"Wrong total_lines: {truncated['total_lines']}"
...
def test_small_files_not_truncated():
    ...
    changed_file = ChangedFile(
        filename="src/small.py", status="modified", additions=50, deletions=0, patch=small_diff
    )
    ...
    print(f"  truncated_files: {context.truncated_files}")
    print("  Status: NOT truncated (correct!)")
