...
def test_hallucination_detection():
    ...
    changed_file = ChangedFile(
        filename="src/auth.py", status="modified", additions=10, deletions=0, patch=fake_diff
    )
    context = PRContext(
        repo_name="test/repo",
        pr_number=1,
    ...
    hallucinated_comment = ReviewComment(
        filename="src/auth.py",
        line=50,  # Hallucination: File only has 10 lines!
        issue="Fake issue",
        suggestion="Fake fix",
        severity="critical",
    )
    hallucinated_file = ReviewComment(
        filename="src/fake_file.py",
        line=1,  # Hallucination: File doesn't exist!
        issue="Fake",
        suggestion="Fake",
        severity="warning",
    )
    ...
    metrics, _evaluations = evaluator.run(context, result)
